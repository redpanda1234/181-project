...
\begin{multicols}{2}
  ...
  \begin{sproof}
    ...
    X_k$. \qedhere
    % represents finitely-many of the $U_n$.
  \end{sproof}
  ...
  \begin{sproof}
    ...
    \begin{enumerate}
      ...
      \item Else, suppose that exactly two of the $n$'s are equal.
        ...
        % \item Suppose that exactly two of the $n$'s are equal. Denote
        %   their common value by $N_{\rm two}$, and the value of the other
        %   $n$ by $N_{\rm one}$. We want to show that $N_{\rm two} < N_{\rm
        %   one}$.
    \end{enumerate}
  ...
  \begin{remark}
    ...
    % \emph{both} $\Sigma$ and
    % $\Gamma$ (defined below) contain some form of ``blank'' character.
    % In the case of $\Sigma$, we denote this as the usual $\epsilon$, and
    % think of $\epsilon$ as being the \emph{empty string}. For gamma,
    % we'll include a $\blank \in \Gamma$, which we call the \emph{blank
    % character} --- essentially, the difference will be that our
    % machine \emph{cannot} write $\epsilon$'s, but it can write
    % $\blank$'s.
  \end{remark}
  ...
  % Compared to the DFA case, there are two conditions we'll loosen:
  % First, we'll allow for a more expanded set of ``configurations'' than
  % just $X_{\rm DFA}$. Second, the evolution map $\Phi$ will allow for
  % more complicated behavior than just scanning linearly from
  % left-to-right.

  % We treat these differences in order.
  ...
  stays ``fixed'' at $0$. % Observe, the requirement $k_1 \leq 0 \leq k_2$
  % requires that we always have one of the $\gamma$'s labeled with
  % $\gamma_0$.

  % except for \emph{one} requirement:
  % \begin{itemize}
  % \item There exists $\ell_{\rm sep} \in \NN$ such that the
  %   longest string of consecutive blank characters in $\gamma_1 \cdot
  %   \gamma_2 \cdots \gamma_n$ has length \emph{at most} $\ell_{\rm
  %   sep}$.
  % \end{itemize}
  % The topological motivation for $\ell_{\rm sep}$ is that it forces our
  % state space to be a $0$-manifold. Fascinatingly, this requirement
  % translates loosely to stipulating that our Turing machine be able to
  % ``determine its configuration'' in finite time, which is important if
  % we want to be able to perform computations deterministically.
  ...
  \begin{proposition}[0-manifold-ness]
  ...
  \begin{sproof}
    ...
    discrete, as desired!
    % $\epsilon$ characters on both sides of
    % $\gamma$, our $\sigma$ is uniquely determined. It follows that
    % $X_{\rm TM}$ is a $0$-manifold, as desired.
  \end{sproof}
  ...
  % Also, we have examples of many well-known problems such that there
  % exists no turing machine
...
\begin{multicols}{2}
  ...
  % These concepts loosely belong to the
  % category of $C^1$ manifolds --- in our general definition of a
  % manifold, we'll loosen the requirements to allow
  % The notion of a manifold subsumes all of
  % these ideas, only we now make some wiggle room for functions like
  % $\abs{x}$ that still
